...
- 更正 / 解除只能从**最新已发布**文档派生；`references` 精确指向被派生的文档
  （`sender,identifier,sent`）。
- 派生消息构成一条以原始标识为根的**衍生链**：每次更正或解除都按全链取下一个序号，
  因此原文的更正是 `-C1`，引用该已发布 C1 的解除是 `-C2`，形成单调递增的单一谱系
  （不区分更正 / 解除类型）。标识确定且 `identifier` 唯一，派生按 `(引用消息, msgType)`
  做幂等：重复点击、并发请求、旧锁重试都不会产生第二份派生或第二条 outbox
  （先做存在性检查，唯一索引兜底）。
- 解除时保留被引用文档的多 info 解析结果（各地区 / 严重度分段）；发布解除后前序
  消息被置为 `superseded`，但**完整版本链仍可查看**（[`Alerts.message_chain/1`](lib/cap_workbench/alerts.ex)
  从链中任意成员返回按序排列的全部消息，并预加载各自的不可变版本）。
- 每一次状态转换（创建 / 提交 / 复核 / 发布 / 被更正或解除）都会写入追加式审计事件。
